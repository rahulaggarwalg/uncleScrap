...
module.exports = {
    ...
    getPickup: callBack => {
        pool.query(
            `select p.id, p.weight, p.message, p.date, p.time, p.is_active, p.created_at, 
            ...
            from pickup p join category c ON p.category_id = c.id left join weight_master w ON p.weight_id = w.id left join address a ON p.address_id = a.id left join city ct ON a.city_id = ct.id;`,
            [],
            (error, results, fields) => {
                if(error) {
                    return callBack(error);
